...
department_data = {} # For departmentwise (/batchwise/termwise) data keeping
...
##    Analyser.All_Courses(printify=True,alphabetically=True,serial=True,terms=True)
    Default_Tools.Batch_Analyser(branch='MECHANICAL ENGINEERING',batch='BT10')
    Default_Tools.Course_Analyser(course='MINI PROJECT',course_term='AUTUMN 2010')
...
##        print(dept)
##        for batches in department_data[dept]:
##            print(batches)
##    print(department_data['CIVIL ENGINEERING']['BT10']['BT10CIV058'])
...
def PDF_Parser(file):
    ...
    def prettify_name(name): # Some names get the foll chars, bad :{
        prettified = name
        for i in (1,2):
            if prettified[-1] == '\xa0':
    ...
    def individual():
        ...
        cur_dep_data = {}
        if cur_branch and cur_branch in department_data:
            if cur_batch in department_data[cur_branch]:
                if not cur_roll in department_data[cur_branch][cur_batch]:
                    department_data[cur_branch][cur_batch][cur_roll] = {}
            else:
                department_data[cur_branch][cur_batch] = {cur_roll:{}}
            cur_dep_data = department_data[cur_branch][cur_batch][cur_roll]
        elif cur_branch:
            department_data[cur_branch] = {cur_batch:{cur_roll:{}}}
            cur_dep_data = department_data[cur_branch][cur_batch][cur_roll]
        all_details['Records'] = {}
        ...
        while good_to_go:
            ...
            if cur_data:
                if cur_data.split()[0] in terms:
                    ...
                    if is_gpa(cur_block[-2]) and not len(cur_block) < 7:
                        ...
                        if cur_term == latest_term[0]: # Latest normal sem
                            all_details['CGPA'] = cgpa_sem
                            all_details['Credits Total'] = creds_tot
                            all_details['EGP Total'] = egp_tot
                            cur_dep_data['CGPA'] = cgpa_sem
                        elif cur_term == latest_term[1]: # Latest sem res after re-exam
                            all_details['CGPA'] = cgpa_sem
                            all_details['Credits Total'] = creds_tot
                            all_details['EGP Total'] = egp_tot
                            cur_dep_data['CGPA'] = cgpa_sem
            ...
            if cur_line == "endstream"+chr(10):
                if cur_branch and cur_roll not in database:
                    database[cur_roll] = all_details
                elif cur_branch: # Requirement for the pro 2 page long grade cards of VIPs
...
class Analyser:
    ...
    def Ranking(marklist, printify=True):
        if printify: # Blank Line to begin with...
            print()
        if marklist:
            if not isinstance(marklist[0],tuple):
                if not printify:
...
class Default_Tools:
    ...
    def Course_Analyser(course='MINI PROJECT',course_term='AUTUMN 2010'):
        marklist = Analyser.Make_Marklist(course=course,course_term=course_term,names=True)
    ...
    def Batch_Analyser(branch='MECHANICAL ENGINEERING',batch='BT10',cg=True,sg=False, term=latest_term[0]):
        marklist = Analyser.Make_Marklist(branch=branch,batch=batch,cg=cg,sg=sg,term=term,names=True)
        Analyser.Mean_Deviation(marklist,printify=True)
        Analyser.Gradify(marklist,printify=True)
        Analyser.Ranking(marklist,printify=True)
